...
v1.0 -> v1.1 changes (dependency-pressure signal):
    Adds _detect_dependency_pressure(): a raw-observable phrase-pattern
    detector for explicit pressure to bypass understanding (e.g. "don't
    teach me", "do all the thinking for me"), distinct from a bare
    solution request. Adds UpdateSignals.dependency_pressure, computes it
    in _update_raw(), and adds a dedicated dD delta so the signal is not
    just visible but actually affects dependency risk. Also threaded into
    capture_snapshot()'s signals dict for local audit/display parity with
    the other signal fields.
"""
...
def _detect_attempt(user_input: str) -> bool:
    ...
    """
    # First gate out pure help requests
...
def _detect_dependency_pressure(user_input: str) -> bool:
    """
    Heuristic: user is explicitly pressuring the system to bypass their own
    understanding — a stronger, more direct signal than a bare solution
    request. This is a raw phrase-pattern observable (same style as
    _detect_help_request), not an inference about intent. It fires
    regardless of length or reasoning markers, since the phrasing itself
    is the signal.
    """
    pressure_phrases = [
        "don't teach me", "do all the thinking for me",
        "copy whatever you say", "without understanding",
        "just do it for me", "don't explain", "no explanation needed",
        "i don't want to learn", "i don't care how", "skip the explanation"
    ]
    lowered = user_input.lower()
    return any(phrase in lowered for phrase in pressure_phrases)
...
class UpdateSignals:
    ...
    dependency_pressure:     bool = False
...
def _update_raw(
    ...
    sig = UpdateSignals(
        ...
        dependency_pressure     = _detect_dependency_pressure(user_input),
    ...
    if sig.user_requested_solution and not sig.user_attempted:
        dD += 0.12
    if sig.dependency_pressure:
        dD += 0.10   # explicit pressure to bypass understanding — stronger
                     # than a bare request, but capped below it so the two
                     # co-occurring on one turn don't blow past the ±0.15
                     # per-variable clamp trivially
...
def route(state: LedgerState, worldview: bool = False) -> ResponseMode:
    ...
    # Repair takes priority over everything else
    if D >= 0.65:
        return ResponseMode.REPAIR

    # Worldview-level question → disciplined meaning-making
    if worldview:
        return ResponseMode.REFLECT

    # High confusion + low agency → reorientation
    if K >= 0.65 and A <= 0.40:
        return ResponseMode.REORIENT

    # High confusion alone → hint + reorientation blend
    if K >= 0.65:
        return ResponseMode.HINT

    # Good agency + low confusion + low dependency → delegate
    if A >= 0.70 and K <= 0.35 and D <= 0.30:
        return ResponseMode.DELEGATE

    # Default: scaffold with hints
    return ResponseMode.HINT
...
REPAIR_TEMPLATES = [
    ...
    # Confusion + dependency combination
    {
...
VISIBILITY_TRANSITIONS = {
    ...
    ResponseMode.HINT:    None,   # No legible transition needed for default mode
...
def get_repair_text(state: LedgerState) -> Optional[str]:
    """
    Return the highest-priority repair template text if triggered, else None.
    """
...
def get_visibility_transition(mode: ResponseMode) -> Optional[str]:
    """
    Return the legible transition string for a mode shift, if any.
    """
...
def build_system_prompt(state: LedgerState, mode: ResponseMode) -> str:
    """
    Build a system prompt that conditions generation on the current ledger state
    and response mode.
    """
    mode_instruction = {
        ResponseMode.REORIENT: (
