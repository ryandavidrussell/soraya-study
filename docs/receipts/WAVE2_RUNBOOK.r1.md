...
grep -Eq '^[^#]*gr\.BrowserState' app.py && grep -Eq '^gradio==4\.' requirements.txt \
  && fail "uncommented gr.BrowserState with a Gradio-4 pin (KL-9 class)"
...
patch (bundled with the KL-1 window) so wave-2 app.py stays byte-identical
to the production lineage. Chain: env var -> git rev-parse -> None.
Study mode must block when resolve_build_sha() returns None (INV: no build
identity, no study)."""
...
## Step 7 — One boring commit, push to GitHub (single-line pushes)

```bash
git add -A
git commit -m "Wave 2: repo becomes deployable; Space-canonical runtime" -m "\
...
## Step 10 — Completion receipt (INV19: partial execution without ABORT is
