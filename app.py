...
def run_turn(
    ...
    # signals is converted to a plain dict here (dataclasses.asdict) so
    # ledger_hook.py / db.py never need to know about the UpdateSignals
    # dataclass shape — they only ever handle dicts.
...
def build_ui():
    ...
    with gr.Blocks(css=CSS + BRAND_CSS, title="Soraya \u2014 A Demonstration") as demo:
        ...
        with gr.Group(elem_classes=["soraya-root"]):
            ...
            # ----------------------------------------------------------------
            # Consent block (shown only in study mode; hidden when mode is off)
            # ----------------------------------------------------------------
            with gr.Group(visible=STUDY_MODE_ENABLED) as consent_group:
                gr.Markdown("## Participation consent")
                consent_html = gr.HTML("")
                cb1 = gr.Checkbox(label=DISCLOSURE_ITEMS[0], value=False)
                cb2 = gr.Checkbox(label=DISCLOSURE_ITEMS[1], value=False)
                cb3 = gr.Checkbox(label=DISCLOSURE_ITEMS[2], value=False)
                pid_box = gr.Textbox(
                    label="Participant ID (leave blank to generate one)",
                    placeholder="Optional",
                )
                consent_btn = gr.Button("I consent — continue", variant="primary")
                consent_msg = gr.Markdown("")

            # ----------------------------------------------------------------
            # Pre-survey (hidden until consent granted)
            # ----------------------------------------------------------------
            with gr.Group(visible=False) as pre_group:
                gr.Markdown(
                    "## Pre-survey\n\n"
                    + _SURVEY_DRAFT_NOTE.format(ver=INSTRUMENT_VERSION)
                )
                pre_widgets = []
                for _key, prompt, kind in PRE_SURVEY_ITEMS:
                    if kind == "likert5":
                        pre_widgets.append(
                            gr.Radio(choices=LIKERT5_LABELS, label=prompt, value=None)
                        )
                    else:
            ...
            # ----------------------------------------------------------------
            # Chat (hidden until pre-survey submitted)
            # ----------------------------------------------------------------
            with gr.Group(visible=not STUDY_MODE_ENABLED) as chat_group:
                with gr.Row():
                    with gr.Column(scale=3):
                        ...
                        with gr.Row():
                            msg_box = gr.Textbox(
                                ...
                                lines=2,
                                scale=8,
                                elem_id="soraya-input",
                            )
                            send_btn = gr.Button("\u2197", scale=1, variant="primary",
                                                 elem_id="soraya-send")
                        with gr.Row():
                            voice_toggle = gr.Checkbox(
                                label="\U0001F50A Voice response (experimental)",
                                value=False,
                            )
                            reset_btn = gr.Button("\U0001F504 Reset session",
                                                  variant="secondary")
                    ...
                    with gr.Column(scale=2):
                        panel = gr.Markdown(
                            ...
                            value=reset_session()[2],
                        )

                gr.HTML(parked_module_html())
                gr.HTML(limit_bar_html())
                finish_btn = gr.Button(
                    "I\u2019m done \u2014 go to post-survey",
                    visible=STUDY_MODE_ENABLED,
                    variant="secondary",
                )

            # ----------------------------------------------------------------
            # Post-survey (hidden until finish clicked)
            # ----------------------------------------------------------------
            with gr.Group(visible=False) as post_group:
                ...
                post_widgets = []
                for _key, prompt, kind in POST_SURVEY_ITEMS:
                    if kind == "likert5":
                        post_widgets.append(
                            gr.Radio(choices=LIKERT5_LABELS, label=prompt, value=None)
                        )
                ...
                post_btn = gr.Button("Submit and finish", variant="primary")
        ...
        def _do_consent(gate, c1, c2, c3, pid):
            ok, message = gate.try_consent(c1, c2, c3, pid or None)
